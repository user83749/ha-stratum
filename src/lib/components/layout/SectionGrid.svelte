<script lang="ts">
	...
	$effect(() => {
		...
		const observer = new ResizeObserver((entries) => {
			...
			if (!entry) return;
			// Debounce via rAF to avoid re-entrant measurements during layout
			// reflows triggered by column count changes (prevents oscillation).
	...
	// Column width (px) — used for X-axis drag snapping
	const cellW = $derived.by(() => {
		const cols = activeColumns;
		const base = Number(section.grid.baseSize ?? 120);
		const pad = Number(section.padding ?? 0);
		const gap = Number(section.grid.gap ?? 0);

		if (!cols || cols < 1) return base;
		if (!gridCtrWidth || gridCtrWidth <= 0) return base;

		const usable = gridCtrWidth - (pad * 2) - ((cols - 1) * gap);

		if (!Number.isFinite(usable) || usable <= 0) return base;
	...
	function flushDragMove() {
		...
		const stepX = Math.round(deltaX / (cellW + gap));
		const stepY = Math.round(deltaY / (cellH + gap));
	...
	function flushResizeMove() {
		...
		const targetW = Math.max(
			1,
			Math.min(activeColumns - resizeState.startLayout.x, resizeState.startLayout.w + Math.round(deltaX / (cellW + gap)))
		);
		const targetH = Math.max(1, resizeState.startLayout.h + Math.round(deltaY / (cellH + gap)));
...
<style>
	...
	.section--editing .section__grid-ctr {
	...
	.section__drag-handle:hover { opacity: 1; color: var(--fg-muted); }
	...
	.section__grid {
		...
		position: relative;
		isolation: isolate;
		/* No CSS transition on grid-template-columns or grid-auto-rows —
		   those properties don't interpolate in browsers and cause
		   ResizeObserver feedback loops that make tiles oscillate. */
	}
	...
	.tile-slot {
		...
		position: relative;
		will-change: auto;
		/* Don't paint-contain tiles so hover shadows/transforms can render outside the cell. */
