<script lang="ts">
	...
	let transition = $state<PageTransitionType>('none');
	...
	<aside class="pe" aria-label="Page editor">
		...
		<div class="pe__body">
			...
			<div class="pe__group">
				...
				/>
				<div class="pe__icon-links">
					<a class="pe__icon-finder" href="https://icon-sets.iconify.design/" target="_blank" rel="noopener noreferrer">Icon Finder ↗</a>
					<button class="pe__icon-finder pe__builtin-btn" onclick={() => builtinOpen = !builtinOpen} type="button">
						Built-in icons {builtinOpen ? '▲' : '▼'}
					</button>
				</div>
				{#if builtinOpen}
					<div class="pe__builtin-list">
						{#each CUSTOM_ICON_NAMES as n}
							<button
								class="pe__builtin-item-box"
								class:pe__builtin-item-box--active={icon === n}
								onclick={() => { icon = n; save({ icon: n }); }}
								title={n}
								type="button"
							>
								<div class="pe__builtin-preview">
									<Icon name={n} size={20} />
								</div>
								<span class="pe__builtin-name">{n}</span>
							</button>
						{/each}
					</div>
				{/if}
...
<style>
	...
	.pe__icon-finder:hover { opacity: 1; }
	...
	.pe__builtin-item-box:hover {
		background: var(--hover);
		color: var(--fg);
		border-color: var(--fg-subtle);
	}

	.pe__builtin-item-box--active {
		border-color: var(--accent);
		color: var(--accent);
	...
	.pe__builtin-item-box:hover .pe__builtin-preview {
		transform: scale(1.1);
	}
