...
export function evaluateCondition(
	...
	const entity = entities[condition.entity_id];
	if (!entity) return false;

	const val = condition.attribute
		? entity.attributes[condition.attribute]
		: entity.state;

	// State check
	if (condition.state !== undefined && val !== condition.state) return false;

	// state_not check
	if (condition.state_not !== undefined && val === condition.state_not) return false;

	// Numeric checks
	const num = parseFloat(String(val));
	if (condition.above !== undefined && (isNaN(num) || num <= condition.above)) return false;
	if (condition.below !== undefined && (isNaN(num) || num >= condition.below)) return false;
...
export function evaluateConditions(
	...
	return conditions.every(c => evaluateCondition(c, entities));
}
